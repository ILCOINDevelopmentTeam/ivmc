...
TEST(tool_commands, run_empty_code)
{
    ...
    const auto exit_code = run(vm, IVMC_FRONTIER, 1, "", "", false, false, out);
...
TEST(tool_commands, run_oog)
{
    ...
    const auto exit_code = run(vm, IVMC_BERLIN, 2, "0x6002600201", "", false, false, out);
    EXPECT_EQ(exit_code, 0);
...
TEST(tool_commands, run_return_my_address)
{
    ...
    const auto exit_code = run(vm, IVMC_HOMESTEAD, 200, "30600052596000f3", "", false, false, out);
    EXPECT_EQ(exit_code, 0);
    EXPECT_EQ(out.str(),
...
TEST(tool_commands, run_copy_input_to_output)
{
    ...
    const auto exit_code =
        run(vm, IVMC_TANGERINE_WHISTLE, 200, "600035600052596000f3",
...
TEST(tool_commands, create_return_1)
{
    ...
    const auto exit_code = run(vm, IVMC_SPURIOUS_DRAGON, 200,
...
TEST(tool_commands, create_copy_input_to_output)
{
    ...
    const auto exit_code =
        run(vm, IVMC_SPURIOUS_DRAGON, 200, "69600035600052596000f3600052600a6016f3", "0c49c4", true,
            false, out);
    EXPECT_EQ(exit_code, 0);
    EXPECT_EQ(
        out.str(),
...
TEST(tool_commands, create_failure_stack_underflow)
{
    ...
    const auto exit_code = run(vm, IVMC_PETERSBURG, 0, "fe", "", true, false, out);
    EXPECT_EQ(exit_code, IVMC_UNDEFINED_INSTRUCTION);
    EXPECT_EQ(out.str(),
...
TEST(tool_commands, create_preserve_storage)
{
    ...
    const auto exit_code =
        run(vm, IVMC_BERLIN, 200, "60bb 6000 55 6a6000546000526001601ff3 6000 52 600b 6015 f3", "",
            true, false, out);
    EXPECT_EQ(exit_code, 0);
...
TEST(tool_commands, bench_add)
{
    ...
    const auto exit_code = run(vm, IVMC_LONDON, 200, "6002 80 01", "", false, true, out);
    EXPECT_EQ(exit_code, 0);
...
TEST(tool_commands, bench_inconsistent_output)
{
    ...
    const auto exit_code = run(vm, IVMC_BYZANTIUM, 200, code, "", false, true, out);
    EXPECT_EQ(exit_code, 0);
