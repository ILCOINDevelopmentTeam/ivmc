...
class NullHost : public ivmc::Host
{
public:
    bool account_exists(const ivmc::address&) const noexcept final { return false; }

    ivmc::bytes32 get_storage(const ivmc::address&, const ivmc::bytes32&) const noexcept final
    {
        return {};
    }

    ivmc_storage_status set_storage(const ivmc::address&,
                                    const ivmc::bytes32&,
                                    const ivmc::bytes32&) noexcept final
    {
        return {};
    }

    ivmc::uint256be get_balance(const ivmc::address&) const noexcept final { return {}; }

    size_t get_code_size(const ivmc::address&) const noexcept final { return 0; }

    ivmc::bytes32 get_code_hash(const ivmc::address&) const noexcept final { return {}; }

    size_t copy_code(const ivmc::address&, size_t, uint8_t*, size_t) const noexcept final
    {
        return 0;
    }

    void selfdestruct(const ivmc::address&, const ivmc::address&) noexcept final {}

    ivmc::result call(const ivmc_message&) noexcept final { return ivmc::result{ivmc_result{}}; }

    ivmc_tx_context get_tx_context() const noexcept final { return {}; }

    ivmc::bytes32 get_block_hash(int64_t) const noexcept final { return {}; }

    void emit_log(const ivmc::address&,
                  const uint8_t*,
                  size_t,
                  const ivmc::bytes32[],
    ...
    ivmc_access_status access_account(const ivmc::address&) noexcept final
    {
        return IVMC_ACCESS_COLD;
    }

    ivmc_access_status access_storage(const ivmc::address&, const ivmc::bytes32&) noexcept final
    {
        return IVMC_ACCESS_COLD;
    }
};
...
TEST(cpp, address)
{
    ...
    EXPECT_EQ(a, ivmc::address{});
    EXPECT_TRUE(is_zero(a));
    EXPECT_FALSE(a);
    EXPECT_TRUE(!a);

    auto other = ivmc_address{};
    other.bytes[19] = 0xfe;
    a = other;
...
TEST(cpp, bytes32)
{
    ...
    EXPECT_EQ(b, ivmc::bytes32{});
    EXPECT_TRUE(is_zero(b));
    EXPECT_FALSE(b);
    EXPECT_TRUE(!b);

    auto other = ivmc_bytes32{};
    other.bytes[31] = 0xfe;
    b = other;
...
#pragma warning(disable : 4309 /* 'static_cast': truncation of constant value */)
    ...
    static_assert(std::hash<ivmc::address>{}({}) == static_cast<size_t>(0xd94d12186c0f2fb7));
    static_assert(std::hash<ivmc::bytes32>{}({}) == static_cast<size_t>(0x4d25767f9dce13f5));

    EXPECT_EQ(std::hash<ivmc::address>{}({}), static_cast<size_t>(0xd94d12186c0f2fb7));
    EXPECT_EQ(std::hash<ivmc::bytes32>{}({}), static_cast<size_t>(0x4d25767f9dce13f5));

    auto ea = ivmc::address{};
    std::fill_n(ea.bytes, sizeof(ea), uint8_t{0xee});
    EXPECT_EQ(std::hash<ivmc::address>{}(ea), static_cast<size_t>(0x41dc0178e01b7cd9));

    auto eb = ivmc::bytes32{};
    std::fill_n(eb.bytes, sizeof(eb), uint8_t{0xee});
    EXPECT_EQ(std::hash<ivmc::bytes32>{}(eb), static_cast<size_t>(0xbb14e5c56b477375));

    const auto rand_address_1 = 0xaa00bb00cc00dd00ee00ff001100220033004400_address;
    EXPECT_EQ(std::hash<ivmc::address>{}(rand_address_1), static_cast<size_t>(0x30022347e325524e));

    const auto rand_address_2 = 0x00dd00cc00bb00aa0022001100ff00ee00440033_address;
    EXPECT_EQ(std::hash<ivmc::address>{}(rand_address_2), static_cast<size_t>(0x17f74b6894b0f6b7));

    const auto rand_bytes32_1 =
        0xbb01bb02bb03bb04bb05bb06bb07bb08bb09bb0abb0bbb0cbb0dbb0ebb0fbb00_bytes32;
    EXPECT_EQ(std::hash<ivmc::bytes32>{}(rand_bytes32_1), static_cast<size_t>(0x4f857586d70f2db9));

    const auto rand_bytes32_2 =
        0x04bb03bb02bb01bb08bb07bb06bb05bb0cbb0bbb0abb09bb00bb0fbb0ebb0dbb_bytes32;
    EXPECT_EQ(std::hash<ivmc::bytes32>{}(rand_bytes32_2), static_cast<size_t>(0x4efee0983bb6c4f5));
...
TEST(cpp, std_maps)
{
    std::map<ivmc::address, bool> addresses;
    addresses[{}] = true;
    ASSERT_EQ(addresses.size(), size_t{1});
    EXPECT_EQ(addresses.begin()->first, ivmc::address{});

    std::unordered_map<ivmc::address, bool> unordered_addresses;
    ...
    std::map<ivmc::bytes32, bool> storage;
    storage[{}] = true;
    ASSERT_EQ(storage.size(), size_t{1});
    EXPECT_EQ(storage.begin()->first, ivmc::bytes32{});

    std::unordered_map<ivmc::bytes32, bool> unordered_storage;
...
TEST(cpp, address_comparison)
{
    ...
    for (size_t i = 0; i < sizeof(ivmc::address); ++i)
    {
        auto t = ivmc::address{};
        t.bytes[i] = 1;
        auto u = ivmc::address{};
...
{
    const auto zero = ivmc::bytes32{};
    auto max = ivmc::bytes32{};
    std::fill_n(max.bytes, sizeof(max), uint8_t{0xff});
    auto z_max = ivmc::bytes32{};
    std::fill_n(z_max.bytes + 8, sizeof(max) - 8, uint8_t{0xff});
    auto max_z = ivmc::bytes32{};
    ...
    for (size_t i = 0; i < sizeof(ivmc::bytes32); ++i)
    {
        auto t = ivmc::bytes32{};
        t.bytes[i] = 1;
        auto u = ivmc::bytes32{};
...
TEST(cpp, literals)
{
    ...
    static_assert(zero_address == ivmc::address{});
    static_assert(zero_hash == ivmc::bytes32{});

    EXPECT_EQ(0_address, ivmc::address{});
    EXPECT_EQ(0_bytes32, ivmc::bytes32{});

    auto a1 = 0xa0a1a2a3a4a5a6a7a8a9d0d1d2d3d4d5d6d7d8d9_address;
    ivmc::address e1{{{0xa0, 0xa1, 0xa2, 0xa3, 0xa4, 0xa5, 0xa6, 0xa7, 0xa8, 0xa9,
                       0xd0, 0xd1, 0xd2, 0xd3, 0xd4, 0xd5, 0xd6, 0xd7, 0xd8, 0xd9}}};
    EXPECT_EQ(a1, e1);

    auto h1 = 0x01020304050607080910a1a2a3a4a5a6a7a8a9b0c1c2c3c4c5c6c7c8c9d0d1d2_bytes32;
    ivmc::bytes32 f1{{{0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x10, 0xa1,
...
TEST(cpp, result)
{
    ...
    {
        auto raw_result = ivmc_result{};
        ivmc_get_optional_storage(&raw_result)->pointer = &release_called;
        EXPECT_EQ(release_called, 0);

        raw_result.output_data = &output;
        raw_result.release = [](const ivmc_result* r) {
            EXPECT_EQ(r->output_data, &output);
            ++*static_cast<int*>(ivmc_get_const_optional_storage(r)->pointer);
        };
        EXPECT_EQ(release_called, 0);

        auto res1 = ivmc::result{raw_result};
        auto res2 = std::move(res1);
        EXPECT_EQ(release_called, 0);

        auto f = [](ivmc::result r) { EXPECT_EQ(r.output_data, &output); };
        f(std::move(res2));
...
TEST(cpp, vm)
{
    auto vm = ivmc::VM{ivmc_create_example_vm()};
    EXPECT_TRUE(vm.is_abi_compatible());

    auto r = vm.set_option("verbose", "3");
    EXPECT_EQ(r, IVMC_SET_OPTION_SUCCESS);

    EXPECT_EQ(vm.name(), std::string{"example_vm"});
    EXPECT_NE(vm.version()[0], 0);

    const auto host = ivmc_host_interface{};
    auto msg = ivmc_message{};
    msg.gas = 1;
    auto res = vm.execute(host, nullptr, IVMC_MAX_REVISION, msg, nullptr, 0);
    EXPECT_EQ(res.status_code, IVMC_SUCCESS);
    EXPECT_EQ(res.gas_left, 1);
}

TEST(cpp, vm_capabilities)
{
    const auto vm = ivmc::VM{ivmc_create_example_vm()};

    EXPECT_TRUE(vm.get_capabilities() & IVMC_CAPABILITY_EVM1);
    EXPECT_FALSE(vm.get_capabilities() & IVMC_CAPABILITY_EWASM);
    EXPECT_FALSE(vm.get_capabilities() & IVMC_CAPABILITY_PRECOMPILES);
    EXPECT_TRUE(vm.has_capability(IVMC_CAPABILITY_EVM1));
    EXPECT_FALSE(vm.has_capability(IVMC_CAPABILITY_EWASM));
    EXPECT_FALSE(vm.has_capability(IVMC_CAPABILITY_PRECOMPILES));
}

TEST(cpp, vm_set_option)
{
    ivmc_vm raw = {IVMC_ABI_VERSION, "", "", nullptr, nullptr, nullptr, nullptr};
    raw.destroy = [](ivmc_vm*) {};

    auto vm = ivmc::VM{&raw};
    EXPECT_EQ(vm.get_raw_pointer(), &raw);
    EXPECT_EQ(vm.set_option("1", "2"), IVMC_SET_OPTION_INVALID_NAME);
}

TEST(cpp, vm_set_option_in_constructor)
{
    static int num_calls = 0;
    const auto set_option_method = [](ivmc_vm*, const char* name, const char* value) {
        ++num_calls;
        EXPECT_STREQ(name, "o");
    ...
    ivmc_vm raw{IVMC_ABI_VERSION, "", "", nullptr, nullptr, nullptr, set_option_method};
    raw.destroy = [](ivmc_vm*) {};

    const auto vm = ivmc::VM{&raw, {{"o", "1"}, {"o", "2"}}};
...
TEST(cpp, vm_move)
{
    ...
    const auto template_vm = ivmc_vm{
        IVMC_ABI_VERSION, "", "", [](ivmc_vm*) { ++destroy_counter; }, nullptr, nullptr, nullptr};
    ...
    {
        // Moving to itself will destroy the VM and reset the ivmc::vm.
        auto v1 = template_vm;
...
{
    auto vm = ivmc::VM{ivmc_create_example_precompiles_vm()};
    EXPECT_EQ(vm.get_capabilities(), ivmc_capabilities_flagset{IVMC_CAPABILITY_PRECOMPILES});
    ...
    auto res = vm.execute(IVMC_MAX_REVISION, msg, nullptr, 0);
    EXPECT_EQ(res.status_code, IVMC_SUCCESS);
...
TEST(cpp, vm_execute_with_null_host)
{
    ...
    auto vm = ivmc::VM{ivmc_create_example_vm()};
    ivmc_message msg{};
    auto res = vm.execute(host, IVMC_FRONTIER, msg, nullptr, 0);
    EXPECT_EQ(res.status_code, IVMC_SUCCESS);
    EXPECT_EQ(res.gas_left, 0);
}
...
TEST(cpp, host)
{
    ...
    const auto& host_interface = ivmc::MockedHost::get_interface();
    auto* host_context = mockedHost.to_context();

    auto host = ivmc::HostContext{host_interface, host_context};

    const auto a = ivmc::address{{{1}}};
    const auto v = ivmc::bytes32{{{7, 7, 7}}};
    ...
    EXPECT_EQ(host.set_storage(a, {}, v), IVMC_STORAGE_MODIFIED);
    EXPECT_EQ(host.set_storage(a, {}, v), IVMC_STORAGE_UNCHANGED);
    EXPECT_EQ(host.get_storage(a, {}), v);

    EXPECT_TRUE(ivmc::is_zero(host.get_balance(a)));

    EXPECT_EQ(host.get_code_size(a), size_t{0});
    EXPECT_EQ(host.get_code_hash(a), ivmc::bytes32{});
    ...
    EXPECT_EQ(host.get_block_hash(0), ivmc::bytes32{});
...
TEST(cpp, host_call)
{
    ...
    auto host = ivmc::HostContext{};  // Use default constructor.
    host = ivmc::HostContext{host_interface, host_context};
    ...
    EXPECT_EQ(recorded_msg1.kind, IVMC_CALL);
    ...
    auto msg = ivmc_message{};
    msg.gas = 1;
    ivmc::bytes input{0xa, 0xb, 0xc};
    msg.input_data = input.data();
    msg.input_size = input.size();

    mockedHost.call_result.status_code = IVMC_REVERT;
    ...
    EXPECT_EQ(recorded_msg2.kind, IVMC_CALL);
    ...
    EXPECT_EQ(ivmc::bytes(recorded_msg2.input_data, recorded_msg2.input_size), input);

    EXPECT_EQ(res.status_code, IVMC_REVERT);
...
TEST(cpp, result_raii)
{
    ...
    release_called = 0;
    auto release_fn = [](const ivmc_result*) noexcept { ++release_called; };

    {
        auto raw_result = ivmc_result{};
        raw_result.status_code = IVMC_INTERNAL_ERROR;
        raw_result.release = release_fn;

        auto raii_result = ivmc::result{raw_result};
        EXPECT_EQ(raii_result.status_code, IVMC_INTERNAL_ERROR);
        EXPECT_EQ(raii_result.gas_left, 0);
        raii_result.gas_left = -1;

        auto raw_result2 = raii_result.release_raw();
        EXPECT_EQ(raw_result2.status_code, IVMC_INTERNAL_ERROR);
        EXPECT_EQ(raw_result.status_code, IVMC_INTERNAL_ERROR);
...
TEST(cpp, result_move)
{
    ...
    release_called = 0;
    {
        auto raw = ivmc_result{};
        raw.gas_left = -1;
        raw.release = release_fn;

        auto r0 = ivmc::result{raw};
    ...
    release_called = 0;
    {
        auto raw1 = ivmc_result{};
        raw1.gas_left = 1;
        raw1.release = release_fn;

        auto raw2 = ivmc_result{};
        raw2.gas_left = 1;
        raw2.release = release_fn;

        auto r1 = ivmc::result{raw1};
        auto r2 = ivmc::result{raw2};
...
{
    auto r = ivmc::result{IVMC_REVERT, 1, nullptr, 0};
    EXPECT_EQ(r.status_code, IVMC_REVERT);
...
TEST(cpp, result_create)
{
    ...
    auto r = ivmc::result{IVMC_FAILURE, -1, output, sizeof(output)};
    EXPECT_EQ(r.status_code, IVMC_FAILURE);
    EXPECT_EQ(r.gas_left, -1);
    ...
    auto c = ivmc::make_result(r.status_code, r.gas_left, r.output_data, r.output_size);
    EXPECT_EQ(c.status_code, r.status_code);
    EXPECT_EQ(c.gas_left, r.gas_left);
    ASSERT_EQ(c.output_size, r.output_size);
    EXPECT_EQ(ivmc::address{c.create_address}, ivmc::address{r.create_address});
...
#define TEST_CASE(STATUS_CODE) \
    ...
    constexpr TestCase test_cases[]{
        TEST_CASE(IVMC_SUCCESS),
        TEST_CASE(IVMC_FAILURE),
        TEST_CASE(IVMC_REVERT),
        TEST_CASE(IVMC_OUT_OF_GAS),
        TEST_CASE(IVMC_INVALID_INSTRUCTION),
        TEST_CASE(IVMC_UNDEFINED_INSTRUCTION),
        TEST_CASE(IVMC_STACK_OVERFLOW),
        TEST_CASE(IVMC_STACK_UNDERFLOW),
        TEST_CASE(IVMC_BAD_JUMP_DESTINATION),
        TEST_CASE(IVMC_INVALID_MEMORY_ACCESS),
        TEST_CASE(IVMC_CALL_DEPTH_EXCEEDED),
        TEST_CASE(IVMC_STATIC_MODE_VIOLATION),
        TEST_CASE(IVMC_PRECOMPILE_FAILURE),
        TEST_CASE(IVMC_CONTRACT_VALIDATION_FAILURE),
        TEST_CASE(IVMC_ARGUMENT_OUT_OF_RANGE),
        TEST_CASE(IVMC_WASM_UNREACHABLE_INSTRUCTION),
        TEST_CASE(IVMC_WASM_TRAP),
        TEST_CASE(IVMC_INSUFFICIENT_BALANCE),
        TEST_CASE(IVMC_INTERNAL_ERROR),
        TEST_CASE(IVMC_REJECTED),
        TEST_CASE(IVMC_OUT_OF_MEMORY),
    };
#undef TEST_CASE
    ...
    {
        std::string expected;
        std::transform(std::cbegin(t.str) + std::strlen("IVMC_"), std::cend(t.str),
                       std::back_inserter(expected), [](char c) -> char {
                           return (c == '_') ? ' ' : static_cast<char>(std::tolower(c));
                       });
        EXPECT_EQ(ivmc::to_string(t.status_code), expected);
        os << t.status_code;
...
#define TEST_CASE(STATUS_CODE) \
    TestCase { STATUS_CODE, #STATUS_CODE }
    constexpr TestCase test_cases[]{
        TEST_CASE(IVMC_FRONTIER),
        TEST_CASE(IVMC_HOMESTEAD),
        TEST_CASE(IVMC_TANGERINE_WHISTLE),
        TEST_CASE(IVMC_SPURIOUS_DRAGON),
        TEST_CASE(IVMC_BYZANTIUM),
        TEST_CASE(IVMC_CONSTANTINOPLE),
        TEST_CASE(IVMC_PETERSBURG),
        TEST_CASE(IVMC_ISTANBUL),
        TEST_CASE(IVMC_BERLIN),
        TEST_CASE(IVMC_LONDON),
        TEST_CASE(IVMC_SHANGHAI),
    };
#undef TEST_CASE

    std::ostringstream os;
    ASSERT_EQ(std::size(test_cases), size_t{IVMC_MAX_REVISION + 1});
    for (size_t i = 0; i < std::size(test_cases); ++i)
    {
        ...
        EXPECT_EQ(ivmc::to_string(t.rev), expected);
        os << t.rev;
        EXPECT_EQ(os.str(), expected);
        os.str({});
    }
...
TEST(cpp, status_code_to_string_invalid)
{
    if (!has_ubsan())
    {
        std::ostringstream os;
        int value = 99;
        const auto invalid = static_cast<ivmc_status_code>(value);
        EXPECT_STREQ(ivmc::to_string(invalid), "<unknown>");
        os << invalid;
...
TEST(cpp, revision_to_string_invalid)
{
    if (!has_ubsan())
    {
        std::ostringstream os;
        int value = 99;
        const auto invalid = static_cast<ivmc_revision>(value);
        EXPECT_STREQ(ivmc::to_string(invalid), "<unknown>");
        os << invalid;
        EXPECT_EQ(os.str(), "<unknown>");
    }
