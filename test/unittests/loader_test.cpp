...
/// The library path expected by mocked ivmc_test_load_library().
extern const char* ivmc_test_library_path;

/// The symbol name expected by mocked ivmc_test_get_symbol_address().
extern const char* ivmc_test_library_symbol;

/// The pointer to function returned by ivmc_test_get_symbol_address().
extern ivmc_create_fn ivmc_test_create_fn;
}
...
protected:
    ...
    static void setup(const char* path, const char* symbol, ivmc_create_fn fn) noexcept
    {
        ivmc_test_library_path = path;
        ivmc_test_library_symbol = symbol;
        ivmc_test_create_fn = fn;
    }

    static void destroy(ivmc_vm*) noexcept { ++destroy_count; }

    static ivmc_set_option_result set_option(ivmc_vm*, const char* name, const char* value) noexcept
    {
...
#pragma GCC diagnostic ignored "-Wconversion"
            return static_cast<ivmc_set_option_result>(-42);
#pragma GCC diagnostic pop
    ...
    static ivmc_vm* create_vm_barebone()
    {
        static auto instance =
            ivmc_vm{IVMC_ABI_VERSION, "vm_barebone", "", destroy, nullptr, nullptr, nullptr};
        ++create_count;
        return &instance;
    }

    /// Creates a VM mock with ABI version different than in this project.
    static ivmc_vm* create_vm_with_wrong_abi()
    {
        constexpr auto wrong_abi_version = 1985;
        static_assert(wrong_abi_version != IVMC_ABI_VERSION);
        static auto instance =
            ivmc_vm{wrong_abi_version, "", "", destroy, nullptr, nullptr, nullptr};
        ++create_count;
        return &instance;
    }

    /// Creates a VM mock with optional set_option() method.
    static ivmc_vm* create_vm_with_set_option() noexcept
    {
        static auto instance = ivmc_vm{
            IVMC_ABI_VERSION, "vm_with_set_option", "", destroy, nullptr, nullptr, set_option};
        ++create_count;
        return &instance;
    }
};
...
static ivmc_vm* create_aaa()
{
    return reinterpret_cast<ivmc_vm*>(0xaaa);
}

static ivmc_vm* create_eee_bbb()
{
    return reinterpret_cast<ivmc_vm*>(0xeeebbb);
}

static ivmc_vm* create_failure()
{
    return nullptr;
}
...
TEST_F(loader, load_nonexistent)
{
    ...
    EXPECT_TRUE(ivmc_load(path, &ec) == nullptr);
    EXPECT_EQ(ec, IVMC_LOADER_CANNOT_OPEN);
    EXPECT_TRUE(ivmc_load(path, nullptr) == nullptr);
}
...
TEST_F(loader, load_long_path)
{
    ...
    ivmc_loader_error_code ec;
    EXPECT_TRUE(ivmc_load(path.c_str(), &ec) == nullptr);
    EXPECT_STREQ(ivmc_last_error_msg(),
                 "invalid argument: file name is too long (5000, maximum allowed length is 4096)");
    EXPECT_TRUE(ivmc_last_error_msg() == nullptr);
    EXPECT_EQ(ec, IVMC_LOADER_INVALID_ARGUMENT);
    EXPECT_TRUE(ivmc_load(path.c_str(), nullptr) == nullptr);
    EXPECT_STREQ(ivmc_last_error_msg(),
                 "invalid argument: file name is too long (5000, maximum allowed length is 4096)");
    EXPECT_TRUE(ivmc_last_error_msg() == nullptr);
}
...
{
    ivmc_loader_error_code ec;
    EXPECT_TRUE(ivmc_load(nullptr, &ec) == nullptr);
    EXPECT_EQ(ec, IVMC_LOADER_INVALID_ARGUMENT);
    EXPECT_STREQ(ivmc_last_error_msg(), "invalid argument: file name cannot be null");
    EXPECT_TRUE(ivmc_last_error_msg() == nullptr);
    EXPECT_TRUE(ivmc_load(nullptr, nullptr) == nullptr);
    EXPECT_STREQ(ivmc_last_error_msg(), "invalid argument: file name cannot be null");
    EXPECT_TRUE(ivmc_last_error_msg() == nullptr);
}
...
{
    ivmc_loader_error_code ec;
    EXPECT_TRUE(ivmc_load("", &ec) == nullptr);
    EXPECT_STREQ(ivmc_last_error_msg(), "invalid argument: file name cannot be empty");
    EXPECT_TRUE(ivmc_last_error_msg() == nullptr);
    EXPECT_EQ(ec, IVMC_LOADER_INVALID_ARGUMENT);
    EXPECT_TRUE(ivmc_load("", nullptr) == nullptr);
    EXPECT_STREQ(ivmc_last_error_msg(), "invalid argument: file name cannot be empty");
    EXPECT_TRUE(ivmc_last_error_msg() == nullptr);
}
...
TEST_F(loader, load_aaa)
{
    ...
    const auto expected_vm_ptr = reinterpret_cast<ivmc_vm*>(0xaaa);

    for (auto& path : paths)
    {
        setup(path, "ivmc_create_aaa", create_aaa);
        ivmc_loader_error_code ec;
        const auto fn = ivmc_load(path, &ec);
        EXPECT_EQ(ec, IVMC_LOADER_SUCCESS);
...
{
    setup("unittests/eee-bbb.dll", "ivmc_create_eee_bbb", create_eee_bbb);
    ivmc_loader_error_code ec;
    auto fn = ivmc_load(ivmc_test_library_path, &ec);
    const auto expected_vm_ptr = reinterpret_cast<ivmc_vm*>(0xeeebbb);
    ASSERT_TRUE(fn != nullptr);
    EXPECT_EQ(ec, IVMC_LOADER_SUCCESS);
    EXPECT_EQ(fn(), expected_vm_ptr);
    EXPECT_TRUE(ivmc_last_error_msg() == nullptr);
...
TEST_F(loader, load_windows_path)
{
    ...
    for (auto& path : paths)
    {
        ...
        setup(should_open ? path : nullptr, "ivmc_create_eee_bbb", create_eee_bbb);
        ...
        {
            EXPECT_EQ(ec, IVMC_LOADER_CANNOT_OPEN);
            EXPECT_STREQ(ivmc_last_error_msg(), "cannot load library");
            EXPECT_TRUE(ivmc_last_error_msg() == nullptr);
        }
    }
}
...
TEST_F(loader, load_symbol_not_found)
{
    ...
    for (auto& path : paths)
    {
        ...
        ivmc_loader_error_code ec;
        EXPECT_TRUE(ivmc_load(ivmc_test_library_path, &ec) == nullptr);
        EXPECT_EQ(ec, IVMC_LOADER_SYMBOL_NOT_FOUND);
        EXPECT_EQ(ivmc_last_error_msg(), "IVMC create function not found in " + std::string(path));
        EXPECT_TRUE(ivmc_last_error_msg() == nullptr);
        EXPECT_TRUE(ivmc_load(ivmc_test_library_path, nullptr) == nullptr);
    }
}

TEST_F(loader, load_default_symbol)
{
    setup("default.ivmc", "ivmc_create", create_aaa);
    ...
    fn = ivmc_load(ivmc_test_library_path, nullptr);
    EXPECT_EQ(fn, &create_aaa);
}

TEST_F(loader, load_and_create_failure)
{
    setup("failure.vm", "ivmc_create", create_failure);

    ivmc_loader_error_code ec;
    auto vm = ivmc_load_and_create(ivmc_test_library_path, &ec);
    EXPECT_TRUE(vm == nullptr);
    EXPECT_EQ(ec, IVMC_LOADER_VM_CREATION_FAILURE);
    EXPECT_STREQ(ivmc_last_error_msg(), "creating IVMC VM of failure.vm has failed");
    EXPECT_TRUE(ivmc_last_error_msg() == nullptr);

    vm = ivmc_load_and_create(ivmc_test_library_path, nullptr);
    EXPECT_TRUE(vm == nullptr);
    EXPECT_STREQ(ivmc_last_error_msg(), "creating IVMC VM of failure.vm has failed");
}
...
{
    setup("abi1985.vm", "ivmc_create", create_vm_with_wrong_abi);

    ivmc_loader_error_code ec;
    auto vm = ivmc_load_and_create(ivmc_test_library_path, &ec);
    EXPECT_TRUE(vm == nullptr);
    EXPECT_EQ(ec, IVMC_LOADER_ABI_VERSION_MISMATCH);
    const auto expected_error_msg =
        "IVMC ABI version 1985 of abi1985.vm mismatches the expected version " +
        std::to_string(IVMC_ABI_VERSION);
    EXPECT_EQ(ivmc_last_error_msg(), expected_error_msg);
    EXPECT_TRUE(ivmc_last_error_msg() == nullptr);
...
{
    setup("path", "ivmc_create", create_vm_with_set_option);

    ivmc_loader_error_code ec;
    auto vm = ivmc_load_and_configure("path", &ec);
    EXPECT_TRUE(vm);
    EXPECT_TRUE(recorded_options.empty());
    EXPECT_EQ(ec, IVMC_LOADER_SUCCESS);

    setup("path", "ivmc_create", create_vm_barebone);

    vm = ivmc_load_and_configure("path,", &ec);
    EXPECT_TRUE(vm);
    EXPECT_TRUE(recorded_options.empty());
    EXPECT_EQ(ec, IVMC_LOADER_SUCCESS);
}
...
TEST_F(loader, load_and_configure_single_option)
{
    ...
    ivmc_loader_error_code ec;
    auto vm = ivmc_load_and_configure("path,o=1", &ec);
    EXPECT_TRUE(vm);
    ...
    recorded_options.clear();
    vm = ivmc_load_and_configure("path,O=2", &ec);
    EXPECT_TRUE(vm);
    ASSERT_EQ(recorded_options.size(), size_t{1});
    EXPECT_EQ(recorded_options[0].first, "O");
...
TEST_F(loader, load_and_configure_uknown_option)
{
    ...
    ivmc_loader_error_code ec;
    auto vm = ivmc_load_and_configure("path,z=1", &ec);
    EXPECT_FALSE(vm);
    ASSERT_EQ(recorded_options.size(), size_t{1});
    EXPECT_EQ(recorded_options[0].first, "z");
    EXPECT_EQ(recorded_options[0].second, "1");
    EXPECT_EQ(ec, IVMC_LOADER_INVALID_OPTION_NAME);
    EXPECT_STREQ(ivmc_last_error_msg(), "vm_with_set_option (path): unknown option 'z'");
    EXPECT_EQ(destroy_count, create_count);

    recorded_options.clear();
    vm = ivmc_load_and_configure("path,x=2,", &ec);
    EXPECT_FALSE(vm);
    ASSERT_EQ(recorded_options.size(), size_t{1});
    EXPECT_EQ(recorded_options[0].first, "x");
    EXPECT_EQ(recorded_options[0].second, "2");
    EXPECT_EQ(ec, IVMC_LOADER_INVALID_OPTION_VALUE);
    EXPECT_STREQ(ivmc_last_error_msg(),
...
TEST_F(loader, load_and_configure_multiple_options)
{
    ...
    ivmc_loader_error_code ec;
    auto vm = ivmc_load_and_configure("path,a=_a,b=_b1,c=_c,b=_b2", &ec);
    EXPECT_TRUE(vm);
    ...
    recorded_options.clear();
    vm = ivmc_load_and_configure("path,a=_a,b=_b2,a=_c,", &ec);
    EXPECT_TRUE(vm);
...
TEST_F(loader, load_and_configure_uknown_option_in_sequence)
{
    ...
    ivmc_loader_error_code ec;
    auto vm = ivmc_load_and_configure("path,a=_a,b=_b,c=_b,", &ec);
    EXPECT_FALSE(vm);
    ASSERT_EQ(recorded_options.size(), size_t{3});
    EXPECT_EQ(recorded_options[0].first, "a");
    EXPECT_EQ(recorded_options[0].second, "_a");
    ...
    recorded_options.clear();
    vm = ivmc_load_and_configure("path,a=_a,x=_b,c=_c", &ec);
    EXPECT_FALSE(vm);
    ...
    EXPECT_EQ(recorded_options[1].second, "_b");
    EXPECT_EQ(ec, IVMC_LOADER_INVALID_OPTION_NAME);
    EXPECT_STREQ(ivmc_last_error_msg(), "vm_with_set_option (path): unknown option 'x'");
    EXPECT_EQ(destroy_count, create_count);
}
...
TEST_F(loader, load_and_configure_empty_values)
{
    ...
    ivmc_loader_error_code ec;
    auto vm = ivmc_load_and_configure("path,flag,e=,flag=,e", &ec);
    EXPECT_TRUE(vm);
    ASSERT_EQ(recorded_options.size(), size_t{4});
...
TEST_F(loader, load_and_configure_degenerated_names)
{
    ...
    ivmc_loader_error_code ec;
    auto vm = ivmc_load_and_configure("path,,,=,,=xxx", &ec);
    EXPECT_TRUE(vm);
...
TEST_F(loader, load_and_configure_comma_at_the_end)
{
    ...
    ivmc_loader_error_code ec;
    auto vm = ivmc_load_and_configure("path,x=x,", &ec);
    EXPECT_TRUE(vm);
    ASSERT_EQ(recorded_options.size(), size_t{1});
    EXPECT_EQ(recorded_options[0].first, "x");
...
TEST_F(loader, load_and_configure_vm_without_set_option)
{
    ...
    ivmc_loader_error_code ec;
    auto vm = ivmc_load_and_configure("path,a=0,b=1", &ec);
    EXPECT_FALSE(vm);
    EXPECT_TRUE(recorded_options.empty());
    EXPECT_EQ(ec, IVMC_LOADER_INVALID_OPTION_NAME);
    EXPECT_STREQ(ivmc_last_error_msg(), "vm_barebone (path) does not support any options");
    EXPECT_EQ(destroy_count, create_count);
    ...
    EXPECT_TRUE(recorded_options.empty());
    EXPECT_EQ(ec, IVMC_LOADER_SUCCESS);
    EXPECT_FALSE(ivmc_last_error_msg());
    ivmc_destroy(vm);
    EXPECT_EQ(destroy_count, create_count);
...
TEST_F(loader, load_and_configure_config_too_long)
{
    ...
    auto vm = ivmc_load_and_configure(config.c_str(), &ec);
    EXPECT_FALSE(vm);
    EXPECT_TRUE(recorded_options.empty());
    EXPECT_EQ(ec, IVMC_LOADER_INVALID_ARGUMENT);
    EXPECT_STREQ(ivmc_last_error_msg(),
...
TEST_F(loader, load_and_configure_error_not_wanted)
{
    ...
    auto vm = ivmc_load_and_configure("path,f=1", nullptr);
    EXPECT_FALSE(vm);
    ASSERT_EQ(recorded_options.size(), size_t{1});
    EXPECT_EQ(recorded_options[0].first, "f");
    EXPECT_EQ(recorded_options[0].second, "1");
    EXPECT_EQ(destroy_count, create_count);
    EXPECT_STREQ(ivmc_last_error_msg(), "vm_with_set_option (path): unknown option 'f'");
    EXPECT_FALSE(ivmc_last_error_msg());
}
